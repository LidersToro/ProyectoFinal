...
class orden{
    ...
    public function modificarEstadoFecha($criterio,$fec,$id,$idOrd)
    {
        $conexion = Conectar::conectarBD();//nos conectamos a la base de datos
        if($conexion != false)
        {
            $sql = "UPDATE orden SET estado = '$criterio', fecha='$fec', idMotoquero = '$id' WHERE idOrden = '$idOrd';";
            echo $sql;
            $stmt=$conexion->prepare($sql);
            $stmt->bind_param('ssi',$criterio, $fec, $id);
            if($stmt->execute())
            {
                $conexion->close();
                return (true);
